...
int main(void){
	...
	struct slot ** board = malloc(boardSize * sizeof(struct slot *));
			...
			slotAdj(board);
			//scoutPlayer(row, column, upLeft, upRight, downLeft, downRight);
			/*for(size_t i = 0;i<boardSize;++i){

				for(size_t j = 0;j<boardSize;++j){

					printf("\nslot(%d, %d) slotAdj: %s",board[i][j].row,board[i][j].column,board[i][j].adj);
				}
			}*/
			fflush(stdout);
	...
	struct Player *player = calloc(maxPlayers, sizeof(struct Player)); // declare a struct array player with size maxPlayers. See cross.h for struct Player declaration;
	...
	for(size_t i=0;i<maxPlayers;i++){ // loop prints values assigned in previous section. Just to make sure everything is working. can be removed in final version.
		...
		printf("\nPlayer current position is: (%d, %d) position type: %s",player[i].pRow,player[i].pCol,player[i].Current_Pos); // prints current position number and slot type
		fflush(stdout);
	} // end loop
		...
		do{
			for(size_t i = 0;i<maxPlayers;++i){
				...
				int pCol = player[i].pCol;
						...
						printf("Player current position is Slot: (%d, %d) ",pRow,pCol); // prints current position makes it easier to keep track of position and make a move
